...
def att_MI_torch(attention, out_shape):
    attention_MI = torch.zeros(
        (len(attention), attention[0].shape[0], attention[0].shape[1])
    )
    full_att = torch.zeros(
        (
            len(attention),
            attention[0].shape[0],
            attention[0].shape[1],
    ...
    for ex_num in range(0, len(out_shape)):
        attention_MI[:, ex_num, :] = mutual_info_torch(
            full_att[:, ex_num, :, 0 : out_shape[ex_num], 0 : out_shape[ex_num]]
        )

    return list(
        torch.quantile(torch.max(attention_MI, dim=-1)[0], q=0.5, dim=0)
        .cpu()
        .detach()
        .numpy()
        .astype(float)
...
def mutual_info_torch(inp):
    ...
    marg_pdf_Y = torch.sum(
        inp
        / torch.sum(inp, dim=(-2, -1))
        .view(inp.shape[0], inp.shape[1], 1, 1)
        .repeat(1, 1, inp.shape[-2], inp.shape[-1]),
        dim=-2,
    )
    marg_pdf_X = torch.sum(
        inp
        / torch.sum(inp, dim=(-2, -1))
        .view(inp.shape[0], inp.shape[1], 1, 1)
        .repeat(1, 1, inp.shape[-2], inp.shape[-1]),
...
class compute_semantic:
    ...
    def coalesce_to_concepts(self, sent_in):
        ...
        return [
            ...
            for tok in toks
            if tok not in list(STOP_WORDS)
            and len(set(string.punctuation).intersection(set(tok))) == 0
        ]
...
class compute_quality:
    def __init__(self, use_roberta=1, distinctness=True, batch_size=64):
        ...
        if self.use_roberta:
            self.config = AutoConfig.from_pretrained(
                "distilroberta-base", output_hidden_states=False, output_attentions=True
            )
            self.tokenizer = AutoTokenizer.from_pretrained(
                "distilroberta-base", do_lower_case=True
            )
            self.model = AutoModel.from_pretrained(
                "distilroberta-base", config=self.config
            )
            self.pad_tok = 1
            self.start_tok = 0
            self.end_tok = 2
    ...
    def compute_MIMA(self, tok_in):
        tok_in = [
            torch.tensor(
                [self.start_tok]
                + self.tokenizer.convert_tokens_to_ids(tok_sent)
                + [self.end_tok]
            )
    ...
    def compute_score(self, sent_in, ref_set=None):
        ...
        sort_ind = sorted(
            range(len(sent_toks)), key=lambda k: len(sent_toks[k]), reverse=True
        )
