...
def create_elipse(x, y, ax, color, n_std):
    ...
    ellipse_obj = shapely_ellipse(
        (x_center, y_center), (width, height), (scale_x, scale_y), 45
    )
...
class smurf_system_analysis:
    ...
    def load_standardized_scores(
        self, estimates_file="smurf/standardize_estimates.txt"
    ):
        ...
        self.stand_MIMA = (
            metric_scores["MIMA"] - estimates.loc[gram_ind, 1]
        ) / estimates.loc[gram_ind, 2]
    ...
    def print_ellipse_intersections(self):
        ...
        for i in cand_systems:
            print(
                ...
                + str(i)
                + " = "
                + str(get_inter(self.ellipse[self.ref_system], self.ellipse[i]))
            )
    ...
    def generate_plot(
        self,
        colors,
        out_file="results/system_plot.png",
        num_random_pts=100,
        seed=10,
        n_std=1.15,
    ):
        ...
        for _ in range(0, num_random_pts):
            ...
            rand_set.append(
                random.choice(
                    [i for i in range(0, num_pts) if i not in estimate_set + rand_set]
                )
            )
